...
export default function AbaConfig({ nomeDaLoja, membrosEquipe }) {
  ...
  // Estados para a gestão da Equipe
  ...
  const salvarConfiguracoes = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        tema: config.tema || "pink",
        cobrarTaxaServico: config.cobrarTaxaServico !== false,
      };

      // O SuperAdmin controla os módulos por fora, então garantimos o merge sem sobrescrever caso seja vazio no state
      await setDoc(ref, payload, { merge: true });
      setConfig((prev) => ({ ...prev, logo: urlDaFoto }));
      setImagemArquivo(null); // Limpa o arquivo após sucesso
  ...
  // ==========================================
  // FUNÇÕES DA EQUIPE (COM CRIAÇÃO NO AUTH)
  // ==========================================
  const adicionarMembro = async (e) => {
    ...
    try {
      ...
      // 1. Verificar se o email já está cadastrado no Auth
      ...
      if (signInMethods.length > 0) {
        alert(
          "Este email já está cadastrado no sistema. Use outro ou adicione como membro existente.",
        );
        return;
      }
      ...
      // 3. Salvar na coleção "equipe" com o UID
      ...
      alert("Membro adicionado à equipe e conta criada com sucesso!");
    } catch (erro) {
      console.error(erro);
  ...
  const removerMembro = async (id) => {
    if (window.confirm("Tem a certeza que deseja remover este acesso?")) {
      try {
        // Opcional: também remover o usuário do Auth? Isso requer permissões de admin.
        // Por segurança, apenas removemos o documento da equipe.
  ...
  return (
    <div className="max-w-4xl animate-in fade-in duration-300 pb-20">
      ...
      <div className="flex gap-2 border-b border-slate-200 mb-8 pb-px overflow-x-auto no-scrollbar">
        ...
        >
          <Users size={18} className="inline mr-2" /> Equipe e Acessos
        </button>
      </div>
      ...
      {["geral", "recebimento", "markup", "fiscal"].includes(abaAtual) && (
        <form onSubmit={salvarConfiguracoes} className="space-y-8">
          ...
          {abaAtual === "geral" && (
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 animate-in fade-in">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2">
                  <label className="block text-sm font-bold text-slate-600 mb-1">
                    Endereço Completo da Loja (Para clientes)
                  </label>
                  <input
                    type="text"
                    name="endereco"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2 mt-4 pt-4 border-t border-slate-100">
                  ...
                  <p className="text-[10px] text-slate-500 mt-2 leading-relaxed max-w-sm">
                    ...
                    tela do código Pix e o pedido irá direto para a sua tela de
                    Triagem aguardando pagamento na entrega.
                  </p>
                </div>
              </div>
            </div>
                ...
                <div className="bg-slate-50 p-5 rounded-2xl border border-slate-200">
                  ...
                  <div className="relative">
                    <input
                      ...
                      value={config.percCustosFixos || 0}
                      onChange={handleChange}
          ...
          {abaAtual === "fiscal" && (
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 animate-in fade-in">
              <h2 className="text-xl font-black text-slate-800 mb-2 flex items-center gap-2">
                <Receipt className="text-blue-500" /> Dados Fiscais da Empresa
              </h2>
              <p className="text-sm text-slate-500 mb-6">
                Preencha os dados oficiais da sua empresa. Estas informações
                serão enviadas no cabeçalho das Notas Fiscais (NF-e/NFC-e).
              </p>

              {!isModuloFiscalAtivo && (
                <div className="bg-amber-50 text-amber-700 p-4 rounded-xl mb-6 flex items-start gap-3 border border-amber-200">
                  <AlertTriangle className="shrink-0 mt-0.5" size={20} />
                  <div>
                    <p className="font-bold">Módulo Fiscal Inativo</p>
                    <p className="text-sm">
                      Os seus dados fiscais podem ser salvos aqui, mas a emissão
                      automática de notas fiscais nas telas de Caixa e Cozinha
                      requer a liberação deste módulo. Entre em contato com o
                      suporte para habilitar os tokens da Sefaz.
                    </p>
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="md:col-span-2">
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    placeholder="Nome oficial de registro na Receita Federal"
                    className="w-full border border-slate-200 p-3.5 rounded-xl outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 bg-slate-50"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    value={config.ie || ""}
                    onChange={handleChange}
                    placeholder="Isento ou numeração da I.E."
                    className="w-full border border-slate-200 p-3.5 rounded-xl outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 bg-slate-50"
                  />
                </div>
              ...
              <h3 className="font-bold text-slate-700 mt-8 mb-4 border-b border-slate-100 pb-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    maxLength="2"
                    placeholder="Ex: SP"
                    className="w-full border border-slate-200 p-3.5 rounded-xl outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 bg-slate-50 uppercase"
                  />
                </div>
              </div>

              {/* ============================================================ */}
              {/* ✅ NOVO CAMPO: Mensagem Complementar Obrigatória */}
              {/* ============================================================ */}
              <div className="mt-8 border-t border-slate-200 pt-6">
                <label
                  htmlFor="mensagemComplementar"
                  className="block text-sm font-bold text-slate-600 mb-1"
                >
                  Mensagem Complementar Obrigatória (NF-e/NFC-e)
                </label>
                <p className="text-xs text-slate-400 mb-2">
                  Texto que será adicionado ao campo "informações
                  complementares" da nota fiscal. Sugestão:{" "}
                  <span className="italic">
                    "DOCUMENTO EMITIDO POR ME OU EPP OPTANTE DO SIMPLES
                    NACIONAL. NAO GERA DIREITO A CREDITO FISCAL DE ISS E IPI."
                  </span>
                </p>
                <textarea
                  id="mensagemComplementar"
                  name="mensagemComplementar"
                  rows="3"
                  value={config.mensagemComplementar || ""}
                  onChange={handleChange}
                  placeholder="Digite a mensagem que deve aparecer na nota fiscal..."
                  className="w-full border border-slate-200 p-3.5 rounded-xl outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 bg-slate-50 resize-y"
                />
              </div>
              {/* ============================================================ */}
            </div>
          ...
          >
            <Save size={24} />{" "}
            {salvando ? "A processar..." : "Salvar Configurações da Loja"}
          </button>
        </form>
      )}

      {/* ABA DE EQUIPE (com correções) */}
      {abaAtual === "equipe" && (
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 animate-in fade-in">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            <div>
              <h2 className="text-xl font-black text-slate-800 flex items-center gap-2">
                <Users className="text-blue-500" /> Equipe e Acessos
              </h2>
