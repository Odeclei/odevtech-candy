...
export default function AbaKanban({ nomeDaLoja }) {
  ...
  const confirmarEmissaoNf = async (e) => {
    ...
    try {
      ...
      // 3. Extrair dados
      const dadosFocus = resultado.dadosFocus || {};
      console.log("📄 dadosFocus recebido:", dadosFocus);

      const chave = dadosFocus.chave_nfe || dadosFocus.chave || null;
      console.log("🔑 Chave extraída:", chave);

      // 4. Validar chave
      if (!chave) {
        console.warn("⚠️ Chave não veio na resposta da Focus!");
        alert(
          "Nota emitida, mas a chave não foi retornada. Entre em contato com o suporte.",
        );
        return; // interrompe o salvamento
      }

      // 5. Montar objeto de atualização
      const updateData = {
        nfEmitida: true,
        statusNFCe: dadosFocus.status || "processando",
        numeroNota: dadosFocus.numero || null,
        nfceChave: chave,
        nfceRef: dadosFocus.ref || null,
        nfceDanfe: dadosFocus.caminho_danfe || null,
        nfceXml: dadosFocus.caminho_xml_nota_fiscal || null,
        nfceEmitidaEm: new Date().toISOString(),
        nfceCanceladaEm: null,
        caminhoPdf: dadosFocus.caminho_danfe || null,
        caminhoXml: dadosFocus.caminho_xml_nota_fiscal || null,
      };
      ...
      alert(`✅ Documento Fiscal enviado para processamento com sucesso!`);
      setModalNfOpen(false);

      // 8. Abrir DANFE se disponível
      if (dadosFocus.caminho_danfe) {
        const urlCompleta = dadosFocus.caminho_danfe.startsWith("http")
          ? dadosFocus.caminho_danfe
          : `https://api.focusnfe.com.br${dadosFocus.caminho_danfe}`;
