...
export default function AbaCaixaBar({ nomeDaLoja }) {
  ...
  useEffect(() => {
    ...
    const unConfig = onSnapshot(doc(db, "lojas", nomeDaLoja), (docSnap) => {
    ...
    const unProdutos = onSnapshot(qProdutos, (snapshot) => {
      setProdutosMenu(snapshot.docs.map((d) => ({ id: d.id, ...d.data() })));
    });
  ...
  const calcularTotaisComanda = (comanda) => {
    ...
    // SÓ APLICA A TAXA SE A CONFIGURAÇÃO PERMITIR
    const aplicarTaxa = configLoja?.cobrarTaxaServico !== false;
    const taxa = aplicarTaxa ? bruto * 0.1 : 0;
    const taxaPaga = aplicarTaxa ? pago * 0.1 : 0;
  ...
  const subtotalSelecionado = (comandaSendoPaga?.itens || []).reduce(
    (acc, item) => acc + (itensSendoPagos[item.id_produto] || 0) * item.preco,
    0,
  );
      ...
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex flex-col md:flex-row gap-4 justify-between items-center">
        ...
        <div className="flex gap-2 w-full md:w-auto">
          ...
          >
            ...
            <option value="aguardando_pagamento">Pediu a Conta</option>
          </select>
          <button
            ...
            return (
              ...
              >
                ...
                <div className="space-y-2 mb-6">
                  ...
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-500">Taxa (10%):</span>
                    <span className="font-bold text-slate-700">
                  ...
                  <div className="flex justify-between items-end pt-2 border-t border-slate-100 mt-2">
                    ...
                    <span className="font-black text-2xl text-amber-600">
                      {formatarDinheiro(totais.saldoDevedor)}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-2">
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      onClick={() => setComandaParaAdicionar(comanda)}
            ...
            <form onSubmit={criarNovaComanda} className="space-y-4">
              <div>
                ...
                >
                  ...
                  <option value="comanda">Comanda Individual (Balcão)</option>
                  <option value="vip">Cliente VIP (Conta Corrente)</option>
                </select>
              </div>
              <div>
                <label className="block text-sm font-bold text-slate-600 mb-1">
          ...
          <div className="bg-white rounded-3xl w-full max-w-md p-6 shadow-2xl overflow-visible">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-black text-slate-800 flex items-center gap-2">
                <ShoppingBag className="text-blue-500" /> Lançar Produto
              </h2>
              <button
            ...
            <p className="text-sm text-slate-500 mb-4">
              Lançando na <b>{comandaParaAdicionar.identificador}</b>
            </p>
            <form onSubmit={adicionarItemNaComanda} className="space-y-4">
              <div>
                <label className="block text-sm font-bold text-slate-600 mb-1">
                ...
                >
                  ...
                  {produtosMenu.map((p) => (
                    <option key={p.id} value={p.id}>
                      {p.nome} - {formatarDinheiro(p.preco)}
                    </option>
                  ))}
                </select>
              </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-3xl w-full max-w-4xl flex flex-col md:flex-row overflow-hidden shadow-2xl h-[85vh] md:h-auto">
            <div className="w-full md:w-3/5 bg-slate-50 flex flex-col border-r border-slate-200">
              <div className="p-6 border-b border-slate-200 bg-white">
                <h2 className="font-black text-xl text-slate-800 flex items-center gap-2">
                  <Receipt className="text-amber-600" /> Dividir Conta:{" "}
              ...
              <div className="p-6 space-y-3 overflow-y-auto flex-1">
                ...
                {(comandaSendoPaga.itens || []).map((item) => {
                  const qtdRestante = item.qtd_total - item.qtd_paga;
                  const qtdSelecionada = itensSendoPagos[item.id_produto] || 0;
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-4 bg-slate-100 p-1.5 rounded-xl">
                        <button
                          onClick={() => alterarItemPagamento(item, -1)}
                        ...
                        <button
                          ...
                          disabled={qtdSelecionada === qtdRestante}
                        >
                          +
                        </button>
                      </div>
            ...
            <div className="w-full md:w-2/5 bg-white flex flex-col relative p-8">
              ...
              >
                <CreditCard size={20} className="inline mr-2" /> Cartão
              </button>
              <button
                disabled={totalAPagarAgora === 0}
              ...
              >
                <Banknote size={20} className="inline mr-2" /> Pix / Dinheiro
              </button>
            </div>
          </div>
        </div>
