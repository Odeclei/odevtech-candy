...
export default function Catalogo() {
  ...
  const [mostrarModalSucessoMesa, setMostrarModalSucessoMesa] = useState(false);
  ...
  useEffect(() => {
    ...
    return onSnapshot(q, (snap) =>
      setProdutosDaLoja(snap.docs.map((d) => ({ id: d.id, ...d.data() }))),
    );
  }, [nomeDaLoja]);
      ...
      if (pInfo) {
        if (pInfo.fichaTecnica && pInfo.fichaTecnica.length > 0) {
          for (const ing of pInfo.fichaTecnica) {
            const insSnap = await getDoc(doc(db, "produtos", ing.id_insumo));
            if (insSnap.exists()) {
              if (
      ...
      try {
        ...
        if (!snapComandas.empty) {
          ...
          carrinho.forEach((cartItem) => {
            ...
            if (idx >= 0) itensAtuais[idx].qtd_total += cartItem.quantidade;
            else
              itensAtuais.push({
  ...
  return (
    ...
    >
      {numeroDaMesa && (
        ...
        >
          <Utensils size={14} className="inline mr-2" /> Mesa {numeroDaMesa}
        </div>
      )}
                ...
                .map((prod) => (
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center border-t pt-4">
                      <span className={`font-black ${tema.texto}`}>
              ...
              {carrinho.map((i) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-3 bg-white px-2 py-1 rounded-xl border shadow-sm">
                    <button onClick={() => alterarQuantidade(i.id, -1)}>
                      <Minus size={14} />
                    </button>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 overflow-y-auto backdrop-blur-sm animate-in fade-in">
          <div className="bg-white p-8 rounded-3xl text-center max-w-md w-full relative">
            ...
            <div className={`p-4 rounded-2xl mb-6 ${tema.bgFundo}`}>
              <p className={`text-xs uppercase font-bold ${tema.texto}`}>
                Valor
              </p>
