...
exports.emitirNFe = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    const emitente = {
      ...
      nome_emitente: lojaData.razaoSocial || "Razão Social não cadastrada",
      ...
      regime_tributario_emitente: parseInt(lojaData.regimeTributario) || 1,
    };
    ...
    console.log("📦 Payload NFe final:", JSON.stringify(payloadFinal, null, 2));
    console.log("###");
    console.log("###");
...
exports.webhookReceiver = functions.https.onRequest(async (req, res) => {
  ...
  try {
    ...
    let querySnapshot = await pedidosRef.where("nfeChave", "==", chave).get();
    ...
    if (!querySnapshot.empty) {
      ...
      const updateData = {
        ...
        nfeAtualizadaEm: new Date().toISOString(),
      ...
      console.log(`✅ Pedido ${pedidoDoc.id} atualizado com status ${status}`);
    } else {
      console.warn(`⚠️ Pedido não encontrado (ref: ${ref}, chave: ${chave})`);
    }
